#!/usr/bin/env python3
"""Repository-wide package inventory for the Pigeon factory.
...
The source lock is ``pigeon/config/upstream-sources.json``: a map of
...
# Stages the rebuild matrix (.github/workflows/rebuild-pigeon.yml) can resolve.
...
def _spec_per_package(root: Path) -> dict[str, Path]:
    specs: dict[str, Path] = {}
    for directory in sorted((root / "pigeon" / "packages").iterdir()):
...
def source_locks(root: Path) -> dict[str, dict]:
    ...
    return load_source_locks(root / "pigeon" / "config" / "upstream-sources.json")
...
def inventory(root: Path) -> list[PackageRecord]:
    ...
    locks = load_source_locks(root / "pigeon" / "config" / "upstream-sources.json")
    return [
        PackageRecord(
            name=name,
            spec=spec,
