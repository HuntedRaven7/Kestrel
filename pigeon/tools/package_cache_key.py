...
Pigeon's only skip mechanism is the published consumer repository, and
...
hit. It does: most specs hardcode `Release: <v>.hum1.pigeon`, and the
...
#     org.kestrel.pigeon.disttag          = <.hum1.pigeon>
    ...
    *,
    ...
    Every field changes what lands in the RPMs:
      ...
      factory_digest    -- reserved: what the root could install from a
                           factory repo (empty: pigeon has none)
      resolved_root     -- what it actually installed, post-resolution
      disttag           -- .hum1.pigeon, which lands in the Release
    """
    payload = json.dumps(
        {
            "schema": SCHEMA,
...
def main() -> int:
    ...
    package_dir = ROOT / "pigeon" / "packages" / args.package
