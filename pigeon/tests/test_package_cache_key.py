...
Pigeon rebuilds every package on every run because its only skip mechanism
...
def test_every_input_changes_the_key():
    ...
    variants = {
        ...
        "disttag": dict(BASE, disttag=".hum2.pigeon"),
        "resolved_root": dict(
...
def test_a_real_recipe_hashes_stably():
    digest = pck.recipe_digest(pck.ROOT / "pigeon" / "packages" / "cpptrace")
    assert re.fullmatch(r"[0-9a-f]{64}", digest)
    assert digest == pck.recipe_digest(pck.ROOT / "pigeon" / "packages" / "cpptrace")
...
def _run_cli(package="cpptrace", root_lines="gcc-1\nglibc-1\n"):
    ...
    try:
        return subprocess.run(
            [sys.executable, str(pck.ROOT / "pigeon" / "tools" / "package_cache_key.py"),
             package, "--buildroot-digest", "sha256:aaa",
             "--disttag", ".hum1.pigeon", "--resolved-root", path],
