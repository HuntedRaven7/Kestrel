...
Rules (PLAN.md §3.2):
  ...
  - Every run writes rpm-factory/reports/<pkg>.json; failed verification leaves the
  ...
  - Vendored extras ("vendored": true) hash a committed file under
    rpm-factory/packages/<pkg>/ — no download (used for cargo vendor trees that
...
REPORTS = ROOT / "rpm-factory" / "reports"
CHUNK = 1024 * 1024
...
def cmd_fetch(pkg: str, output: str | None, stage_into: str | None = None, verify_staged: bool = False) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix=f"kestrel-{pkg}-") as tmp:
        ...
        if verify_staged:
            ...
            staged_path = ROOT / "rpm-factory" / "packages" / pkg / archive.name
            ...
            for extra in entry.get("extra_sources", []) or []:
                xstaged = ROOT / "rpm-factory" / "packages" / pkg / extra.get("filename", "")
...
def cmd_record(pkg: str, output: str | None) -> int:
    ...
    if entry.get("vendored"):
        ...
        staged = ROOT / "rpm-factory" / "packages" / pkg / filename if filename else None
    ...
    with tempfile.TemporaryDirectory(prefix=f"kestrel-{pkg}-") as tmp:
        ...
        # Also copy to package directory for the build's Source0 lookup
        pkg_dir = ROOT / "rpm-factory" / "packages" / pkg
        if pkg_dir.exists():
            shutil.copy(archive, pkg_dir / archive.name)
        entry["sha512"] = digest
...
def cmd_fetch_vendored(pkg: str, entry: dict, recorded: str, output: str | None) -> int:
    """Verify staged primary and declared secondary bytes for vendored inputs."""
    filename = entry.get("filename", "")
    staged = ROOT / "rpm-factory" / "packages" / pkg / filename if filename else None
    if staged is None or not staged.is_file():
    ...
    extras: list[dict] = []
    extras_ok = True
    with tempfile.TemporaryDirectory(prefix=f"kestrel-{pkg}-extras-") as temporary:
        workdir = Path(temporary)
        for extra in entry.get("extra_sources", []) or []:
            result = fetch_extra(pkg, extra, str(entry.get("version", "")), workdir, output, None)
            extras.append(result)
            if result["ok"]:
                print(f"OK: extra {result['filename']} verified")
            elif _extra_is_sidecar(result.get("filename", "")):
                print(f"WARN: extra {result['filename']} — {result['reason']} (non-fatal)")
            else:
                print(f"FAIL: extra {result['filename']} — {result['reason']}")
                extras_ok = False
    ...
    write_report(pkg, {
        "package": pkg, "version": entry.get("version"), "vendored": True,
        "sha512": digest, "expected": recorded, "extra_sources": extras,
        "ok": ok and extras_ok,
    })
    if not ok:
...
def fetch_extra(pkg: str, extra: dict, version: str, workdir: Path,
    ...
    Vendored extras ("vendored": true) hash a committed file under
    rpm-factory/packages/<pkg>/ and never download.
    """
...
def record_extra(pkg: str, extra: dict, version: str, workdir: Path) -> dict:
    ...
    Vendored extras hash the committed file under rpm-factory/packages/<pkg>/.
    """
    filename = extra.get("filename", "")
    if not filename:
        return {"ok": False, "filename": filename,
