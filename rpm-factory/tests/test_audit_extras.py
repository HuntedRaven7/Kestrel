...
def _setup(tmp_path, monkeypatch, spec_body: str, extras: list):
    root = tmp_path
    pkgdir = root / "rpm-factory" / "packages" / "widg"
    ...
    (root / "rpm-factory" / "config").mkdir(parents=True)
    (root / "rpm-factory" / "config" / "upstream-sources.json").write_text(
        json.dumps(data))
    monkeypatch.setattr(au, "ROOT", root)
    monkeypatch.setattr(au, "PKGS", root / "rpm-factory" / "packages")
    return pkgdir
