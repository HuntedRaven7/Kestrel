"""Tests for the RPM factory package inventory (single source of truth).
...
def test_source_locks_rejects_unknown_stage(tmp_path):
    ...
    try:
        pi.load_source_locks(tmp_path / "rpm-factory" / "config" / "upstream-sources.json")
...
def test_source_locks_defaults_to_stage_2(tmp_path):
    ...
    locks = pi.load_source_locks(tmp_path / "rpm-factory" / "config" / "upstream-sources.json")
...
def test_render_packit_config_matches_inventory(tmp_path):
    ...
    assert "rpm-factory/packages/fish/fish.spec" in out
    assert "rpm-factory/packages/color-filesystem/color-filesystem.spec" in out